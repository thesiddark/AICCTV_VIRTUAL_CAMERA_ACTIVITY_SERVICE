        ...
        try:
            if count < 30:
                ...
                if f== True:
                    ...
                    #
                    #     db.insert(qry)
                    mediapath = "C:\\Users\\Acer\\PycharmProjects\\FACETRACK_MAJLIS\\media"
                    from datetime import datetime
                    filename = datetime.now().strftime("%Y%m%d%H%M%S") + ".jpg"

                    ms = mediapath + filename
                    db = Database()
                    id = db.insert("INSERT INTO `myapp_violence` (`date`,`time`,`photo`) VALUES (CURDATE(),CURTIME(),'" + "/media/" + filename + "')")
